...
// Display aliases (show user's names)
const DISPLAY_NAME = {
...
export default function App() {
    ...
    if (idx >= 2) {
      ...
      if (first && picked && isFirstStrikerTroop(picked)) {
        ...
        if (pickedS > firstS || pickedH > firstH) {
          ...
          setWarningMsg(
            `${label} (${picked}) has higher BASE strength (${fmtInt(pickedS)}) and/or BASE health (${fmtInt(pickedH)}) than your First striker (${first}, ${fmtInt(firstS)} / ${fmtInt(firstH)}).\n\nSuggestion: choose a stronger First striker troop.`
          );
  ...
  const showResults = () => {
    ...
    lines.push({
      role: "Wall Killer",
      troop: wallKillerTroop || "-",
      required: wallKiller.requiredTroops,
    });
    for (const s of perStriker)
      lines.push({ role: s.label, troop: s.troopName || "-", required: s.requiredTroops });

    // Build ordered output using RESULT_ORDER; only include troops that are present and selected.
    const counts = new Map();
    for (const l of lines) {
      if (!l?.troop) continue;
      const key = normName(l.troop);
      const prev = counts.get(key) || { name: l.troop, required: 0 };
      counts.set(key, { name: l.troop, required: prev.required + Math.floor(l.required || 0) });
    }

    const ordered = [];
    for (const displayName of RESULT_ORDER) {
      const tryKeys = [
        normName(displayName),
        // common alias keys
        displayName === "Catapults IV" ? normName("Catapult IV") : null,
        displayName === "Catapults V" ? normName("Catapult V") : null,
        displayName === "Spearmen V" ? normName("Spearman V") : null,
        displayName === "Swordsmen V" ? normName("Swordsman V") : null,
        displayName === "Manticore" ? normName("Fearsome Manticore") : null,
        displayName === "Epic Monster Hunter" ? normName("Superior Epic Monster Hunter") : null,
      ].filter(Boolean);

      let hit = null;
      for (const k of tryKeys) {
        const v = counts.get(k);
        if (v && v.required > 0) { hit = v; break; }
      }
      if (hit) {
        const shown = DISPLAY_NAME[hit.name] || displayName;
        ordered.push({ troop: shown, required: Math.floor(hit.required) });
      }
    }
              ...
              <div style={{ display: "grid", gap: 10 }}>
                <label style={{ display: "grid", gap: 6 }}>
                  ...
                  <select
                    ...
                    onChange={(e) => handleTroopChange(idx, e.target.value)}
                    style={inputStyle}
                  >
                    {opts.map((name) => (
