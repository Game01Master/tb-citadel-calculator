...
export default function App() {
  ...
  const optionsForIdx = (idx) => {
    ...
    if (mode === MODE_WITH && idx === 2) {
      const filteredNoManticore = filtered.filter((n) => normName(n) !== normName("Manticore"));
      return ["", ...filteredNoManticore];
    }
  ...
  const perStriker = useMemo(() => {
    ...
    return STRIKER_LABELS.map((label, idx) => {
      ...
      // Third Striker: round to be as close as possible to Target HP
