...
// --- Helpers for base stats ---
const getBaseStrength = (troopName) => {
  if (!troopName) return 0;
  const t = (tbData?.troops || []).find(x => x.name === troopName);
  return Number(t?.baseStrength ?? t?.strength ?? 0);
};

const getBaseHealth = (troopName) => {
  if (!troopName) return 0;
  const t = (tbData?.troops || []).find(x => x.name === troopName);
  return Number(t?.baseHealth ?? t?.health ?? 0);
};
...
const isFirstStrikerTroop = (troopName) => {
  ...
  const list =
    mode === "WITH"
      ? tbData.firstStrikerAllowed.WITH
      : tbData.firstStrikerAllowed.WITHOUT;
  return list.includes(troopName);
};
