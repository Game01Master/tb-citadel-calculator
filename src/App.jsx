...
export default function App() {
  ...
  const [strikerBonusPct, setStrikerBonusPct] = useState(() => Array(9).fill(0));
  const [firstHealthBonusPct, setFirstHealthBonusPct] = useState(0);
  ...
  const [wallKillerBonusPct, setWallKillerBonusPct] = useState(0);
  ...
  const handleModeChange = (newMode) => {
    ...
    setStrikerBonusPct(() => Array(9).fill(0));
    setFirstHealthBonusPct(0);
    setGroupBonusPct({
      CORAX: 0,
      PHOENIX: 0,
      PHH_SPEAR: 0,
  ...
  const setTroopAt = (idx, name) => {
    ...
    if (g) {
      setStrikerBonusPct((prev) => {
        const next = [...prev];
        next[idx] = toNum(groupBonusPct[g] ?? 0);
        return next;
      });
    }
    setCalcOutput(null);
  ...
  const setBonusAt = (idx, v) => {
    ...
    if (g) {
      setGroupBonusPct((prev) => ({ ...prev, [g]: val }));
      setStrikerBonusPct((prev) => {
        const next = [...prev];
        for (let i = 0; i < strikerTroops.length; i++) {
          if (getBonusGroup(strikerTroops[i]) === g) next[i] = val;
        }
        return next;
      });
    } else {
