...
export default function App() {
  ...
  // Order index for dropdown sorting (matches the Calculate/output troop order for the current mode).
  const nonWallOrderIndex = useMemo(() => {
    const m = new Map();
    for (let i = 0; i < nonWallPool.length; i++) m.set(normName(nonWallPool[i]), i);
    return m;
  }, [nonWallPool]);
  ...
  const optionsForIdx = (idx) => {
    ...
    // For Third Striker and all Cleanup cards (idx >= 2), show troops in the same order as the Calculate/output order.
    if (idx >= 2) {
      filtered.sort((a, b) => {
        const ia = nonWallOrderIndex.get(normName(a)) ?? 9999;
        const ib = nonWallOrderIndex.get(normName(b)) ?? 9999;
