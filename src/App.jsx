...
// Display order for Calculate results (user-defined)
const RESULT_ORDER = [
  "Wyvern",
  "Warregal",
  "Jago",
...
export default function App() {
  ...
  const showResults = () => {
    // Build base lines from current selections (existing behavior)
    const lines = [];
    lines.push({
      role: "Wall Killer",
    ...
    // --- Display-only transform (do NOT change calculations) ---
    // Keep only troops that were actually selected before Calculate
    const counts = new Map();
    const add = (troop, n) => {
      if (!troop || troop === "-" || !Number.isFinite(n)) return;
      const k = normName(troop);
      counts.set(k, (counts.get(k) || 0) + Math.floor(n));
    };
    for (const l of lines) add(l.troop, l.required);

    // Order by user's list
    const ordered = [];
    for (const name of RESULT_ORDER) {
      const k = normName(name);
      if (counts.has(k)) ordered.push({ troop: name, required: counts.get(k) });
    }

    setCalcOutput({
      modeLabel: mode === MODE_WITHOUT ? "Without M8/M9" : "With M8/M9",
      citadelLabel: `Elven ${citadelLevel}`,
      lines: ordered, // keep same field name to avoid touching other logic
    });
    setResultsOpen(true);
  };
            ...
            {calcOutput.lines.map((l, i) => (
              ...
              >
                <div
                  style={{
                    fontWeight: 900,
                    color: isDark ? "#d7e3ff" : theme.text,
                    fontSize: 14,
                    lineHeight: 1.25,
                    flex: "1 1 auto",
                  }}
