...
// Results display order (user-defined)
const RESULT_ORDER = [
  "Wyvern",
  "Warregal",
  "Jago",
...
// Display aliases (show user's names even if internal canonical name differs)
const DISPLAY_NAME = {
  "Catapult IV": "Catapults IV",
  "Catapult V": "Catapults V",
  "Spearman V": "Spearmen V",
  "Swordsman V": "Swordsmen V",
  "Fearsome Manticore": "Manticore",
  "Superior Epic Monster Hunter": "Epic Monster Hunter",
};
...
export default function App() {
  ...
  const showResults = () => {
    // Aggregate required troops by troop name (only selected troops)
    const counts = new Map();

    const addCount = (troopName, n) => {
      if (!troopName || troopName === "-" || !Number.isFinite(n)) return;
      const key = normName(troopName);
      const prev = counts.get(key) || { name: troopName, required: 0 };
      counts.set(key, { name: troopName, required: prev.required + Math.floor(n) });
    };

    // Wall killer (only if selected)
    if (wallKillerTroop) addCount(wallKillerTroop, wallKiller.requiredTroops);

    // Strikers (only if selected)
    for (const s of perStriker) {
      if (s.troopName) addCount(s.troopName, s.requiredTroops);
    }

    // Build ordered output using RESULT_ORDER; only include troops that are present in counts.
    const ordered = [];
    for (const displayName of RESULT_ORDER) {
      const key = normName(displayName);
      const hit = counts.get(key);

      // Try common canonical forms if user-facing name differs (plural/singular etc.)
      let resolved = hit;
      if (!resolved) {
        // map known aliases to canonical keys
        const aliasKey =
          displayName === "Catapults IV" ? normName("Catapult IV") :
          displayName === "Catapults V" ? normName("Catapult V") :
          displayName === "Spearmen V" ? normName("Spearman V") :
          displayName === "Swordsmen V" ? normName("Swordsman V") :
          displayName === "Manticore" ? normName("Fearsome Manticore") :
          displayName === "Epic Monster Hunter" ? normName("Superior Epic Monster Hunter") :
          null;
        if (aliasKey) resolved = counts.get(aliasKey);
      }

      if (resolved && resolved.required > 0) {
        const internalName = resolved.name;
        const shown = DISPLAY_NAME[internalName] || displayName;
        ordered.push({ troop: shown, required: Math.floor(resolved.required) });
      }
    }

    setCalcOutput({
      modeLabel: mode === MODE_WITHOUT ? "Without M8/M9" : "With M8/M9",
          ...
          <div style={{ display: "grid", gap: 10 }}>
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  setWallKillerTroop(e.target.value);
                  setCalcOutput(null);
                  setResultsOpen(false);
                }}
                style={inputStyle}
              >
                {wallKillerPool.map((n) => (
            ...
            <Card key={idx} title={`${idx + 1}. ${s.label}`} theme={theme}>
              <div style={{ display: "grid", gap: 10 }}>
                <label style={{ display: "grid", gap: 6 }}>
                  ...
                  >
                    {opts.map((name) => (
                      <option key={name || "__blank__"} value={name}>
                        {name === "" ? "—" : name}
                      </option>
                    ))}
                  </select>
        ...
        {calcOutput ? (
          <>
            ...
            {calcOutput.troops.map((l, i) => (
              ...
              >
                <div style={{ fontWeight: 900, color: theme.text }}>{l.troop}</div>
                <div style={{ marginTop: 4, color: theme.text }}>
                  <b>{fmtInt(l.required)}</b>
                </div>
              </div>
            ))}
          </>
