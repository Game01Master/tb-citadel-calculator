...
// Fallback First Striker allowed lists (used if TB.firstStrikerAllowed is missing in JSON)
const DEFAULT_FIRST_STRIKER_ALLOWED = {
  WITH: ["Corax II", "Corax I", "Griffin VII"],
  WITHOUT: ["Corax I", "Griffin VII", "Griffin VI", "Griffin V", "Royal Lion I"],
};
...
const citadelKeys = Object.keys(TB.citadels ?? {});
  ...
  const firstStrikerAllowed = TB.firstStrikerAllowed ?? DEFAULT_FIRST_STRIKER_ALLOWED;
...
const [resultsOpen, setResultsOpen] = useState(false);
  ...
  const setTroopAt = (idx, name) => {
    ...
    // Show warning ONLY if the picked troop is on the First Striker list for the current mode,
    // and it has higher BASE strength OR higher BASE health than the selected First Striker.
    if (idx >= 2 && name) {
      const firstTroop = strikerTroops[0];
      if (firstTroop && isFirstStrikerTroop(name)) {
        const firstStr = getBaseStrength(firstTroop);
        const firstHp = getBaseHealth(firstTroop);
        const pickedStr = getBaseStrength(name);
        const pickedHp = getBaseHealth(name);

        if (pickedStr > firstStr || pickedHp > firstHp) {
          const pickedLabel = STRIKER_LABELS[idx] || "Striker";

          setWarningMsg(
            `Invalid striker order.\n\n${pickedLabel} (${name})\nBase Strength: ${pickedStr}\nBase Health: ${pickedHp}\n\nFirst striker (${firstTroop})\nBase Strength: ${firstStr}\nBase Health: ${firstHp}\n\nSuggestion: choose a stronger First striker troop.`
          );

          // Clear the picked striker selection and its bonus
          setStrikerTroops((prev) => normalize(prev.map((v, i) => (i === idx ? "" : v))));
        ...
        {calcOutput ? (
          <>
            ...
            {calcOutput.lines.map((l, i) => (
              ...
              >
                <div style={{ fontWeight: 900, color: theme.text }}>{l.role}</div>
                <div style={{ color: theme.subtext }}>{l.troop}</div>
                <div style={{ marginTop: 4, color: theme.text }}>
                  Required troops: <b>{fmtInt(l.required)}</b>
                </div>
              </div>
            ))}
          </>
