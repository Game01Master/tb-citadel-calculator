...
export default function App() {
  ...
  const showResults = () => {
    ...
    const add = (name, n) => {
      if (!name || name === "-" || !Number.isFinite(n)) return;
    ...
    const pickedFirst = strikerTroops[0] || "";
    const pickedSecond = strikerTroops[1] || "";
    ...
    const take = (name) => {
      ...
      firstSecond.push({ troop: name, required: counts.get(k) });
      counts.delete(k);
    };

    // Keep First + Second at the top (attack order)
    take(pickedFirst);
    take(pickedSecond);

    // Third + Cleanup (and anything else remaining) in user-defined order
    const rest = [];
    for (const name of RESULT_ORDER) {
      const k = normName(name);
      if (counts.has(k)) {
        rest.push({ troop: name, required: counts.get(k) });
        counts.delete(k);
      }
    }

    // Any leftover selected troops not in RESULT_ORDER (should be rare)
    for (const [k, v] of counts.entries()) {
      rest.push({ troop: k, required: v });
    }
      ...
      >
        {calcOutput ? (
          <>
            ...
            <div style={{ height: 10 }} />
            {calcOutput.firstSecond && calcOutput.firstSecond.length > 0 && (
              <>
                {calcOutput.firstSecond.map((l, i) => (
                  <div
                    key={`fs-${i}`}
                    style={{
                      padding: "10px 0",
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10 }}>
                      ...
                      <div style={{ fontWeight: 900, color: theme.text }}>{fmtInt(l.required)}</div>
                    </div>
                  </div>
                ))}
              </>
