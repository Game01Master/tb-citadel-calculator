...
export default function App() {
  ...
  const normalize = (current) => {
    ...
    if (mode === MODE_WITH && next[2] && normName(next[2]) === normName("Manticore")) next[2] = "";
    const seen = new Set();
    for (let i = 0; i < 9; i++) {
  ...
  const optionsForIdx = (idx) => {
    ...
    else if (idx === 2 && mode === MODE_WITH) pool = nonWallPool.filter((n) => normName(n) !== normName("Manticore"));
