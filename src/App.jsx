...
export default function App() {
  ...
  const showResults = () => {
    // Keep all existing rules/calculations; only change the RESULT DISPLAY content & order.
    ...
    for (const s of perStriker) {
      if (s?.troopName && Number.isFinite(s?.requiredTroops)) {
    ...
    // First striker, Second striker: show first (if selected), in the order of the attack.
    for (const idx of [0, 1]) {
      const troopName = perStriker[idx]?.troopName;
      const req = perStriker[idx]?.requiredTroops;
      if (troopName && req > 0) {
        const k = normName(troopName);
        out.push({ troop: troopName, required: Math.floor(req) });
        used.add(k);
      }
    }

    // Third striker + Cleanup: display in user-defined order list.
    for (const name of RESULT_ORDER) {
      const k = normName(name);
      if (used.has(k)) continue;
      if (counts.has(k)) {
        const req = counts.get(k);
        if (req > 0) out.push({ troop: name, required: Math.floor(req) });
      }
    }
